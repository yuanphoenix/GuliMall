...
public class WebController {
  ...
  public String transfer(
    ...
    return "redirect:http://member.gulimall.com/memberOrder.html?page=1";
  }


  @GetMapping("/memberOrder.html")
  public String memberOrder(Model model, @RequestParam("page") Integer page,
      @LoginUser MemberEntityVo memberEntityVo) {
    OrderInfoTo orderInfoTo = new OrderInfoTo();
    orderInfoTo.setPage(page);
    R list = memberService.test(memberEntityVo, orderInfoTo);
    var c = list.getPage(new TypeReference<Page<OrderInfoTo>>() {
    });
    log.info(c.toString());
